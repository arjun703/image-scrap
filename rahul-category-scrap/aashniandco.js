...
const launchOptions = {
  ...
  for (const row of data) {
    ...
    if(!link.includes('aashniandco')) continue
      ...
      // const browser = await puppeteer.launch({ headless: false });
       // await page.goto(link);
        ...
        categories = await page.evaluate(() => {
            ...
            const catWrapper = document.querySelector('[data-info="patternsFilter"]')

            if(catWrapper){
              
              allSpans  = Array.from(catWrapper.querySelectorAll('span.label'))
          ...
          console.log(`index: ${processedLink}, link: ${link}, categories: ${categories}`)
  ...
  fs.writeFileSync('op-all-aashniandco.csv', csv, 'utf8');
