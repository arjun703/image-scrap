  ...
  try {
    ...
    while (count < maxCount && imageCount < maxImages ) {
      ...
      imageCount = await page.evaluate(() => {
        return document.querySelectorAll('.main-image').length;
      });
	...
	let imageURLs  = [];
     ...
     imageURLs = await page.evaluate(() => {
        return Array.from(document.querySelectorAll('.main-image')).map(imgNode => imgNode.getAttribute('src'));
      });
...
async function processExcel(filePath) {
  ...
  for (const row of data) {
    ...
    if(!link.includes('ogaan')) continue
  ...
  fs.writeFileSync('all-ogaan.csv', csv, 'utf8');
