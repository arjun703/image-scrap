...
async function fetchLabourers(filePath) {
  ...
  for (let i = 0; i < links.length; i += batchSize) {
    ...
    const promises = batchLinks.map(async (link) => {
      try {
        ...
        processedLink++;
        console.log(`Labourers: ${allLabourers.length+10000}`);
  ...
  fs.writeFileSync('./op/java-labourers-2.csv', csv, 'utf8');
  fs.writeFileSync('./op/erroneous-java-labourers-2.json', JSON.stringify(erroneous), 'utf8');
